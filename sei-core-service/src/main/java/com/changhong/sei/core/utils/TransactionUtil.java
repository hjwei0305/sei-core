 ...
 * @version 1.0.00  2020-01-07 16:55
 */
public class TransactionUtil {
    ...
    private static PlatformTransactionManager getTransactionManager() {
        return ApplicationContextHolder.getBean(PlatformTransactionManager.class);
    }
    ...
    private static void onCommit(Runnable runner, boolean before) {
        if (!TransactionSynchronizationManager.isSynchronizationActive() || IN_COMMIT.get() != null) {
            runner.run();
            return;
        }
        TransactionSynchronizationManager.registerSynchronization(new OnCommitSynchronization(runner, before));
    }
    ...
    private static <T extends Runnable> T getOnCommit(Class<T> clazz, boolean before) {
        if (!TransactionSynchronizationManager.isSynchronizationActive()) {
            return null;
        }
        List<TransactionSynchronization> tss = TransactionSynchronizationManager.getSynchronizations();
        for (TransactionSynchronization synchronization : tss) {
            if (synchronization instanceof OnCommitSynchronization) {
                ...
                if (before == onCommit.before && onCommit.runner.getClass() == clazz) {
     ...
     */
    private static class OnCommitSynchronization extends TransactionSynchronizationAdapter {
